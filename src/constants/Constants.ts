export const Constants = {
    ...
    build: {
        assets: (type, hash = true, extension = '[ext]', onlyPath = false) => `build/assets/${type}/${onlyPath ? '' : `[name]${hash ? '-[hash]' : ''}.${extension}`}`,
        URL: (path = undefined) => `https://hamyarsystem.com${path ? `/${path}` : ''}`
    },
    forms: {
        URL: (formID) => `https://hamyarsystem.com/blog/gravityformsapi/forms/${formID}/submissions`
    }
};